    ...
    <div v-for="group in groupedTasks" :key="group.title">
      ...
      <div :class="{ hidden: !(isOpen[group.title] ?? true) }">
        <div v-for="(d, index) in group.tasks" :key="d.name">
          ...
          <div
            class="flex items-center rounded p-0.5 transition hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-400"
            v-if="
              d.is_group
            "
          >
            ...
            <div style="height: 50px; border-left: 1px solid gray; margin: 0 5px;"></div>
          </div>
          </div>
          <!-- Updated by Omar Jaber -->
