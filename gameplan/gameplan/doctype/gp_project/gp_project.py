...
@frappe.whitelist()
def get_projects():
    Project = DocType('GP Project')
    Member = DocType('GP Member')
    GuestAccess = DocType('GP Guest Access')

    # Query to check membership in team
    member_exists = (
        frappe.qb.from_(Member)
        .select(Member.name)
        .where(Member.parenttype == 'GP Team')
    ...
    # Query to check membership in project
    project_member_exists = (
        frappe.qb.from_(Member)
        .select(Member.name)
        .where(Member.parenttype == 'GP Project')
    ...
    query = frappe.qb.from_(Project).select(Project.name, Project.title)

    # Filter based on private or public and membership
    query = query.where(
        (Project.is_private == 0) | 
        ((Project.is_private == 1) & ExistsCriterion(member_exists) & ExistsCriterion(project_member_exists))
    )

    # Additional filter for guest users
    if is_guest():
        guest_projects = frappe.qb.from_(GuestAccess).select(GuestAccess.project).where(GuestAccess.user == frappe.session.user)
        query = query.where(Project.name.isin(guest_projects))
